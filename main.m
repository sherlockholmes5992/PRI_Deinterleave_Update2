...
clear emCfg; 
emCfg(1).type = 'Fixed';     emCfg(1).PRI = 157e-5;             emCfg(1).RF = 5e9;   emCfg(1).PW = 2e-6;   emCfg(1).jitter = 0;   emCfg(1).t_start = 0.002;      emCfg(1).p_missing = 0;          emCfg(1).toa_error = 2e-6;
emCfg(2).type = 'Fixed';     emCfg(2).PRI = 157e-5;             emCfg(2).RF = 5e9;   emCfg(2).PW = 2e-6;   emCfg(2).jitter = 0;   emCfg(2).t_start = 0.0015; emCfg(2).p_missing = 0;          emCfg(2).toa_error = 2e-6;
% emCfg(3).type = 'Staggered'; emCfg(3).PRI = [106e-5, 112e-5, 90.8e-5]; emCfg(3).RF = 5e9;  emCfg(3).PW = 6e-6;   emCfg(3).jitter = 0;   emCfg(3).t_start = 0.0005; emCfg(3).p_missing = 0;          emCfg(3).toa_error = 2e-6;
% emCfg(4).type = 'Jittered';  emCfg(4).PRI = 50e-5;              emCfg(4).RF = 5e9;  emCfg(4).PW = 9e-6;   emCfg(4).jitter = 0.1; emCfg(4).t_start = 0.0030; emCfg(4).p_missing = 0;          emCfg(4).toa_error = 2e-6;
% emCfg(5).type = 'Fixed';     emCfg(5).PRI = 157e-5;             emCfg(5).RF = 5e9;   emCfg(5).PW = 2.4e-6; emCfg(5).jitter = 0;   emCfg(5).t_start = 0.0035; emCfg(5).p_missing = 0;          emCfg(5).toa_error = 2e-6;
...
algoParams.x_emp = 0.15;      % Empirical scaling factor for the optimal threshold curve bounds
algoParams.k_emp = 0.2;        % Decay coefficient controlling the dynamic threshold slope curvature
algoParams.W = 1e-6;
algoParams.C_test = 1; 
...
[pdwData, initialSDIF] = generate_pulses(emCfg, t_sim, algoParams);
...
% plot_results(pdwData, final_sequences, initialSDIF, length(emCfg), algoParams);
% 

% %% --- STEP 4: GENERATE AUDIT REPORT & VISUAL PLOTS ---
export_results(pdwData, separated_sequences, extracted_flags);
plot_results(pdwData, separated_sequences, initialSDIF, length(emCfg), algoParams);